...
class Telnet {
  ...
  protected $NAWS;
	 ...
	 * @return void
	 */
	public function __construct($p_host = '127.0.0.1',
                              $p_port = '23',
                              $p_timeout = 10,
                              $p_prompt = '$',
                              $p_stream_timeout = 1)
  {
    ...
    $this->NAWS    = chr(31);
		...
		$this->setPrompt($p_prompt);
		$this->setStreamTimeout($p_stream_timeout);
	}
  ...
  public function setWindowSize($p_cols, $p_rows)
  {
    $this->nbCols = $p_cols;
    $this->nbRows = $p_rows;
  }
	...
	public function connect() {
		...
		$this->socket = @fsockopen($this->host, $this->port, $this->errno, $this->errstr, (double)$this->timeout);

		if (!$this->socket) {
			throw new Exception("Cannot connect to $this->host on port $this->port : ($this->errno) $this->errstr");
		}
		...
		if ($c != $this->IAC)
    {
        ...
        if ($opt == $this->NAWS) {
          ...
          fwrite($this->socket, $this->IAC . $this->SB . $this->NAWS . chr(0) . chr($this->nbCols) . chr(0) . chr($this->nbRows) . $this->IAC . $this->SE);
        } else {
